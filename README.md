# CFSG Playing Cards

The Classification Theorem of Finite Simple Groups states that every finite simple group is either a member of 18 specific infinite families of simple groups, namely,

- cyclic groups of prime order,
- alternating groups of degree at least 5,
- 16 other infinite families known as the simple groups of Lie type,

or one of 26 specific groups called the "sporadic groups".

## Sources

- [Wilson - The Finite Simple Groups](https://link.springer.com/book/10.1007/978-1-84800-988-2)
- [List of Finite Simple Groups](https://www.wikipedia.com/en/articles/List_of_finite_simple_groups)
- [Mathieu Groups M_11 & M_12 - Steiner System Construction](https://warwick.ac.uk/fac/sci/maths/people/staff/holmes/final_project.pdf)
- [20 The Golay codes](https://www.sciencedirect.com/science/chapter/bookseries/abs/pii/S0924650908705452)
- [Janko’s Sporadic Simple Groups: a bit of history](https://carmamaths.org/meetings/mathsandcomputation/pdfs/mathscomp2015-taylor.pdf)
- [Dynkin Diagram](https://mathworld.wolfram.com/DynkinDiagram.html)
  - Lie group visualizations
- [Frans Marcelis - Mathematics of physics](https://fgmarcelis.wordpress.com/)
  - projective plane visualizations
- [Groups: index](https://weddslist.com/rmdb/gindex.php)
  - Cayley graph visualizations
- [O'Nan Moonshine and Arithmetic](https://arxiv.org/abs/1702.03516)
  - connection between O'Nan group and $E_{15}$ elliptic curve
- [Introduction to Sporadic Groups](https://arxiv.org/pdf/1101.3055)
  - diagram of subgroup relations of sporadic groups
